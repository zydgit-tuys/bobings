...
serve(async (req) => {
    ...
    try {
        ...
        const { data: existingDebit } = await supabase
            ...
            .select('id')
            .eq('event_type', 'confirm_purchase')
            .eq('side', 'debit')
            .limit(1)
            .maybeSingle();
        ...
        if (!existingCredit) {
            await supabase.from('journal_account_mappings').insert({
                ...
                event_context: null,
                side: 'credit',
                account_id: accHutang.id,
                priority: 10,
                is_active: true
            });
        }

        return new Response(JSON.stringify({ success: true, message: "Use V2 Purchase Seeded (Manual Check)" }), {
